...
- **Reuse over rebuild**: `routes/results.py` serves the existing self-contained HTML report (iframe-embedded) and `<SYM>_chart_data.json`; `dashboard.js` builds the interactive Overview from that JSON; `reports_index.py` scans `report_output_dir` for the history gallery. `_paths.py` guards every file path with a strict symbol regex + `report_output_dir` containment check (traversal-safe). Frontend reuses the report's design tokens. Tests: `tests/test_web_*.py` (FastAPI `TestClient`, `analyze_stock` mocked). New settings `web_host`/`web_port`.
