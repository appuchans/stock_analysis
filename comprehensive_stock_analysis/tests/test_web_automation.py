...
class TestScheduleEndpoints:
    def test_create_and_list_schedule(self):
        resp = client.post("/api/schedules", json={"target": "watchlist", "cron_expr": "0 18 * * 1-5"})
    ...
    def test_create_normalizes_symbol_target_uppercase(self):
        resp = client.post("/api/schedules", json={"target": "aapl", "cron_expr": "0 * * * *"})
        assert resp.status_code == 201
        assert resp.json()["target"] == "AAPL"

    def test_create_invalid_target_422(self):
        resp = client.post("/api/schedules", json={"target": "../evil", "cron_expr": "0 * * * *"})
        assert resp.status_code == 422

    def test_create_invalid_cron_422(self):
        resp = client.post("/api/schedules", json={"target": "watchlist", "cron_expr": "garbage"})
        assert resp.status_code == 422

    def test_toggle_schedule(self):
        row = client.post("/api/schedules", json={"target": "AAPL", "cron_expr": "0 * * * *"}).json()
        resp = client.post(f"/api/schedules/{row['id']}/toggle", params={"enabled": False})
...
class TestRuleEndpoints:
    def test_create_and_list_rule(self):
        resp = client.post("/api/rules", json={
            "symbol": "AAPL", "rule_type": "price_above", "threshold": 200.0,
        })
        assert resp.status_code == 201
        body = resp.json()
    ...
    def test_create_price_rule_without_threshold_422(self):
        resp = client.post("/api/rules", json={"symbol": "AAPL", "rule_type": "price_above"})
        assert resp.status_code == 422

    def test_create_recommendation_changed_rule_without_threshold_ok(self):
        resp = client.post("/api/rules", json={
            "symbol": "AAPL", "rule_type": "recommendation_changed",
        })
        assert resp.status_code == 201

    def test_create_invalid_symbol_422(self):
        resp = client.post("/api/rules", json={
            "symbol": "../evil", "rule_type": "recommendation_changed",
        })
        assert resp.status_code == 422

    def test_create_invalid_rule_type_422(self):
        resp = client.post("/api/rules", json={"symbol": "AAPL", "rule_type": "not_a_real_type"})
        assert resp.status_code == 422

    def test_toggle_rule(self):
        row = client.post("/api/rules", json={
            "symbol": "AAPL", "rule_type": "recommendation_changed",
        }).json()
