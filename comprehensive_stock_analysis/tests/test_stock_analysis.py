...
def sample_volume_data():
    ...
    return [
        {"volume": volumes[i], "timestamp": dates[i].to_pydatetime()}
        for i in range(100)
    ]
...
class TestRiskCalculatorTool:
    ...
    def _make_price_data(self, n: int = 60):
        ...
        return [
            {"close": float(prices[i]), "timestamp": dates[i].isoformat()}
            for i in range(n)
        ]
...
class TestAPIFailureHandling:
    ...
    @patch("src.stock_analysis.tools.cache._get_redis", return_value=None)
    @patch("yfinance.Ticker")
    def test_yahoo_finance_handles_empty_company_officers(
        self, mock_ticker, _mock_redis
    ):
